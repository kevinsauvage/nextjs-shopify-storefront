...
| Area                              | Verdict                                                                                                           |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Core commerce + P0 gaps           | **Shipped** — catalog, cart, checkout, auth, orders, Pages CMS, activation, order detail, cart extras, newsletter |
| Shopify API surface still unwired | Blog, metaobjects, localization/markets, customer metafields, pickup/delivery, Multipass                          |
| Biggest remaining levers          | Content-led SEO (blog + metaobjects), Markets/i18n, retention loops (reviews, recently-viewed, loyalty)           |
...
| `getShopMetaobject(s)` + `getShopMetaobjectByHandle` (`shopQueries.graphql`) | Metaobjects as CMS (hero banners, promos, lookbooks, size charts, FAQ blocks) | **P1 — pick one use-case (homepage hero / promo banner)** and ship it |
...
| Metaobjects                                                   | Queries exist, no rendering                        | P1 — merchandiser-editable sections                             |
...
5. **Recently viewed + smarter recommendations** — localStorage rail + existing
   `productRecommendations` on cart/search empty states, not only PDP.

6. **Search upgrade** — recent searches, popular terms (metaobject), zero-result fallback
   (best-sellers + contact CTA), analytics event on search.

7. **Wishlist hardening** — guest → login merge (server metafield wins vs. union — decide +
   document), share-wishlist link, move-to-cart.

8. **GTM Enhanced Ecommerce** — `view_item`, `add_to_cart`, `begin_checkout`, `purchase`
   (post-checkout thank-you is on Shopify — use web pixels or checkout extensibility note).

### P2 — Differentiators (needs scoping)

9. **Subscriptions** — query `sellingPlanGroups`, variant-level plan selector, manage-subscription
   link to customer portal.

10. **Bundles / shop-the-look** — combined listings or manual bundle section with a single
    `cartLinesAdd`.

11. **Pickup + delivery estimate** — buyer identity + location inventory pre-checkout.

12. **Loyalty / referral** — metafields-backed points display; provider (Yotpo/LoyaltyLion) vs. custom.

13. **Multipass SSO** — only if an external IdP exists; adds secret handling + `Ask first` review.

14. **PWA / offline cart draft, AI search/sizing, visual search** — validate demand first.
...
| Sprint 4   | P1 #5–#7 (recently viewed, search, wishlist) | Retention + conversion depth            |
| Sprint 5   | P1 #4, #8 (reviews, GTM Enhanced Ecommerce)  | Social proof + measurement              |
